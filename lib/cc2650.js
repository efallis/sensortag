...
CC2650SensorTag.prototype.convertAudioData = function(data, callback) {
  ...
  var audioData = new Buffer(data)
  //console.log(audioData);
  var decoded = Adpcm.decodeAdpcm(audioData);
  //console.log(decoded)

  data[dataIndex] = decoded;
  dataIndex += 1;
  ...
  var avg = total / (decoded.length - missing);
  //console.log(avg);
  console.log(audioData);
  
  // Send adpcm data, not converted, Python script is better at conversion (TI script)
  fs.writeFile('.audio.csv', audioData, (err) => {});
